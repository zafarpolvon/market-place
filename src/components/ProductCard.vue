  ...
  <div class="cart__box shadow-xl">
    ...
    >
      ...
      <router-link
        ...
        :key="product.id"
        :to="{ name: 'Add', params: { id: product.id } }"
        v-if="listOne"
        @click="listOne = false"
        class="fast__review"
      >
      ...
      <icon-love
        :isLove="IS_FAVORITE(product.id)"
        @add="ADD_FAVORITE(product)"
      />
    </div>
    <div class="cart__info">
      <div class="flex justify-between mt-3">
        <a class="cart__cat">{{ product.category.name }}</a>
        <a class="cart__brand" href="#">{{ product.brand.name }}</a>
      </div>
      <div class="cart__title">
        ...
        <div class="cart__price">
          <h5>{{ product.price }} рубль</h5>
          <p>{{ product.price_old }} рубль</p>
        </div>
      </div>
      <div class="cart__add">
        <button>
...
export default {
  ...
  computed: {
    ...mapGetters("product", ["IS_FAVORITE"]),
    productName: (vm) => `${vm.product?.name || ""}`.slice(0, 20),
  },

  methods: {
    ...mapActions("product", ["ADD_FAVORITE"]),
